-- R2-06A shared serialization for durable legacy-game writes and deletes.
-- Additive repository migration only. Do not apply outside a separately approved release.
--
-- Lock contract:
--   * every mutation derives one transaction-scoped advisory lock from the
--     canonical game ID and the fixed "laxhornet:legacy-game:" namespace;
--   * the lock is acquired before any tombstone or game-row read/mutation;
--   * one RPC mutates only one game ID, so there is no multi-key lock ordering;
--   * unrelated IDs retain independent 64-bit advisory-lock keys;
--   * the trigger repeats the same lock and tombstone check for direct/legacy
--     writes. A transaction may reacquire its own advisory lock safely.
...
begin
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('laxhornet:legacy-game:' || new.id, 0)
  );
...
comment on function lh_sync_private.reject_tombstoned_game_write() is
  'Defense-in-depth guard for direct legacy writes. Uses the same per-game transaction lock as the guarded write and durable delete RPCs.';
...
begin
  ...
  -- First mutation lock in this RPC. Every later read/write is ordered behind
  -- the durable delete RPC for this exact game ID.
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('laxhornet:legacy-game:' || incoming.id, 0)
  );
...
comment on function public.laxhornet_sync_game(jsonb) is
  'Guarded legacy-game upsert. Acquires the shared per-game transaction lock before checking tombstones or mutating games.';
...
begin
  ...
  -- First mutation lock in this RPC. This is the same namespaced 64-bit key
  -- used by laxhornet_sync_game and the direct-write trigger.
  perform pg_catalog.pg_advisory_xact_lock(
...
comment on function public.laxhornet_delete_game_durable(jsonb) is
  'Transactional durable legacy-game delete. Acquires the shared per-game transaction lock before checking tombstones or game rows.';
